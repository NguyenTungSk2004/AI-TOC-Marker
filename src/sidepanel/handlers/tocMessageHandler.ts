    ...
    chrome.runtime.onMessage.addListener((request, _sender, sendResponse) => {
        if (request.type === 'TOC_DATA') {
            ...
            if (currentTabUrl && platform !== ChatPlatform.UNKNOWN) {
                ...
                console.log(`[TOC] Cập nhật platform state: ${platformState.getPlatformName()} tại ${currentTabUrl}`);
            ...
            if (currentJSON !== previousTOCJSON) {
                if (previousURL !== currentTabUrl) {
                    tocState.closeAll();
                    console.log(`[TOC] URL thay đổi từ ${previousURL} sang ${currentTabUrl}, đóng tất cả groups`);
                }
                renderTOC(main, toc);
